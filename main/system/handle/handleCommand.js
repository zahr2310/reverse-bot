module.exports = function({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const isOperator = OPERATOR.includes(senderID);
    const isOwner = OWNER.includes(senderID);
    const isBotSelf = senderID === String(api.getCurrentUserID());
    const isPrivileged = isAdminBot || isOperator || isOwner || isBotSelf;
    ...
    if (command && (command.config.name.toLowerCase() === commandName.toLowerCase()) && (!APPROVED.includes(threadID) && !isPrivileged && approval)) {
      return api.sendMessage(notApproved, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
          });
    }
    if (typeof body === 'string' && body.startsWith(PREFIX) && (!APPROVED.includes(threadID) && !isPrivileged && approval)) {
      return api.sendMessage(notApproved, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
          });
    ...
    // ── ADMIN ONLY: silently ignore non-admins ──
    if (adminOnly && !isPrivileged) return;
    ...
    if (commandBanned.get(threadID) || commandBanned.get(senderID)) {
      if (!isAdminBot && !isOperator) {
        const banThreads = commandBanned.get(threadID) || [],
          banUsers = commandBanned.get(senderID) || [];
        if (command && banThreads.includes(command.config.name))
          return api.sendMessage(global.getText("handleCommand", "commandThreadBanned", command.config.name), threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000))
            return api.unsendMessage(info.messageID);
          }, messageID);
        if (command && banUsers.includes(command.config.name))
    ...
    if (command && command.config && command.config.category && command.config.category.toLowerCase() === 'nsfw' && !global.data.threadAllowNSFW.includes(threadID) && !isAdminBot)
    ...
    if (isOperator) permssion = 3;
    else if (isOwner) permssion = 4;
    else if (isAdminBot) permssion = 2;
    else if (!isAdminBot && ryuko && Find) permssion = 1;
